...
class TCPDF {
	...
	public function Footer() {
		...
		if (empty($this->pagegroups)) {
			$pagenumtxt = $this->getAliasNumPage();/* $w_page. *///.' / '.$this->getAliasNbPages();
		} else {
			$pagenumtxt = $this->getPageNumGroupAlias();/* $w_page. *///.' / '.$this->getPageGroupAlias();
		}
		$this->SetY($cur_y);
		//Print page number
		if ($this->getRTL()) {
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
		...
		switch($tag['value']) {
			...
			case 'table': {
				...
				if (isset($table_el['attribute']['border']) AND ($table_el['attribute']['border'] > 0)) {
					$tbcolor= isset($table_el['attribute']['bordercolor'])? TCPDF_COLORS::convertHTMLColorToDec($table_el['attribute']['bordercolor'], $this->spot_colors):array(0,0,0);
					// set default border
					$border = array('LTRB' => array('width' => $this->getCSSBorderWidth($table_el['attribute']['border']), 'cap'=>'square', 'join'=>'miter', 'dash'=> 0, 'color'=>$tbcolor));
				} else {
					$border = 0;
				}
	...
	*/
	 ...
	 * @public
	 */
	public function ImageSVG($file, $x='', $y='', $w=0, $h=0, $link='', $align='', $palign='', $border=0, $fitonpage=false,$content=false) {
		if ($this->state != 2) {
			 return;
		}
		// reset SVG vars
