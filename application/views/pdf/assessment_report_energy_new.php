...
$results=$assessor=$comname=$comp_id=$kkcomp_name_graph=array();
$kkkk=1;
foreach($results_comp as $results_comps){
	$comp_id=$results_comps['comp_id'];
	$kkcomp_name_graph[$comp_id]="C".($kkkk)." ".$results_comps['comp_name'];
	$kkkk++;
}
/* echo "<pre>";
print_r($kkcomp_name_graph); */
...
foreach($kkcomp_name_graph as $key => $kkcomp_name_graphs){
	$para11.=$kkcomp_name_graphs.'<br>';	
}
$para11.='</td>
			</tr>
		</tbody>
...
<div style="font-family: Helvetica;font-size:11px;line-height: 1.5;color:#646464;">
	...
	<thead><tr height="30" bgcolor="#59b0ea" align="center" class="row header" style="color:#fff;">
		<th width="7%">S.No</th>
		<th width="25%">Competency/Skill</th>
		<th width="15%">Level Requirement</th>';
	...
	<tbody>';
	$asscompinfo=UlsAssessmentReport::getassessment_admin_competency($ass_id,$pos_id,'');	
	$results_comp=array();
	foreach($asscompinfo as $ass_comp_infos){
		$comp_id=$ass_comp_infos['comp_def_id'];
		$req_scale_id=$ass_comp_infos['req_scale_id'];
		$results_comp[$comp_id]['comp_id']=$comp_id;
		$results_comp[$comp_id]['comp_name']=!empty($ass_comp_infos['comp_def_name_alt'])?$ass_comp_infos['comp_def_name_alt']:$ass_comp_infos['comp_def_name'];
		$results_comp[$comp_id]['req_scale_id']=$ass_comp_infos['req_scale_id'];
		$results_comp[$comp_id]['req_scale_name']=$ass_comp_infos['req_scale_name'];
		$results_comp[$comp_id]['assessment_id']=$ass_comp_infos['assessment_id'];
		$results_comp[$comp_id]['position_id']=$ass_comp_infos['position_id'];
	}
	$kk=0;
	$nn=1;
	foreach($results_comp as $comp_def_id=>$result){
		$final_admin_id=$_SESSION['user_id'];
		$bgcolor=($kk%2==0)?"#deeaf6":"#ffffff";
		$kk++;
		$para11_com.='<tr bgcolor="'.$bgcolor.'">
		<td width="7%">'.$nn.'</td>
		<td width="25%">'.$result['comp_name'].'</td>
		<td width="15%">'.$result['req_scale_name'].'</td>';
		foreach($assmethods as $ass_methodss){
			if($ass_methodss['assessment_type']=='TEST'){
				$ass_comp_test=UlsAssessmentReport::admin_position_assessed_competency_level_test_new($_REQUEST['ass_id'],$_REQUEST['pos_id'],$result['comp_id'],$_REQUEST['emp_id']);
				$para11_com.='<td width="12%">'.(!empty($ass_comp_test['ass_ave'])?round($ass_comp_test['ass_ave'],2):"-").'</td>';
			}
			if($ass_methodss['assessment_type']=='INBASKET'){
				$ass_comp_inbasket=UlsAssessmentReport::admin_position_assessed_competency_level_inbasket_new($_REQUEST['ass_id'],$_REQUEST['pos_id'],$result['comp_id'],$_REQUEST['emp_id']);
				$para11_com.='<td width="12%">'.(!empty($ass_comp_inbasket['ass_ave'])?round($ass_comp_inbasket['ass_ave'],2):"-").'</td>';
			}
			if($ass_methodss['assessment_type']=='CASE_STUDY'){
				$ass_comp_case=UlsAssessmentReport::admin_position_assessed_competency_level_case_new($_REQUEST['ass_id'],$_REQUEST['pos_id'],$result['comp_id'],$_REQUEST['emp_id']);
				$para11_com.='<td width="12%">'.(!empty($ass_comp_case['ass_ave'])?round($ass_comp_case['ass_ave'],2):"-").'</td>';
			}
		}
		$para11_com.='</tr>';
		$nn++;
...
if(!empty($compid)){
	foreach($compid as $compids){
		...
		$para_data.='<br><br><br><span align="left" style="font-family: Helvetica;font-size: 13px;float:right;font-weight: bold;">Competency:</span><span align="left" style="text-transform: uppercase;font-family: Helvetica;font-size: 13px;color:#36a2eb; float:right;font-weight: bold;font-style: italic;">'.$com_detail.'</span><br><br>';
		if(count($element_pro)>0){
			foreach($element_pro as $element_pros){
				...
				$para_data.='Programs: '.$element_pros['program_name'].'<br>';
			}
		}
		else{
			$para_data.='Specific development needs have not been identified';
		}
		if(!empty(trim($dev[$comp_d[0]]))){
			$para_data.='<br><b>Assessor Comments:</b><br><span style="font-style: italic;">'.$dev[$comp_d[0]].'</span>';
		}
