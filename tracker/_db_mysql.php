...
function db_get_tasks($parent) {
    global $db;
	$date=date("Y-m-01");
    $str = 'SELECT * FROM task WHERE parent_id = :parent and start>="'.$date.'" ORDER BY ordinal, ordinal_priority desc';
    if ($parent == null) {                                           
        $str = str_replace("= :parent", "is null", $str);
        $stmt = $db->prepare($str);
    }
