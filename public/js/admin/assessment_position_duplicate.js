function getpositiontest(assessment_id){
	...
	xmlhttp.onreadystatechange=function()
	{
		...
		{
			document.getElementById('dup_position').innerHTML=xmlhttp.responseText;
		}
	}
	xmlhttp.open("GET",BASE_URL+"/admin/get_position_duplicate?assessment_id="+assessment_id+"&position_id="+position_id,true);
	xmlhttp.send();
}

function getduppositiontest(assessment_id){
	var position_id=document.getElementById('ass_position_id').value;
	var dup_position_id=document.getElementById('dup_position_id').value;
	var xmlhttp;
	if (window.XMLHttpRequest)
	{// code for IE7+, Firefox, Chrome, Opera, Safari
	xmlhttp=new XMLHttpRequest();
	...
	xmlhttp.open("GET",BASE_URL+"/admin/get_position_duplicate_questions?assessment_id="+assessment_id+"&position_id="+position_id+"&du_position_id="+dup_position_id,true);
	xmlhttp.send();
}
